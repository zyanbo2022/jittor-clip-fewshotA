...
import jittor
import jittor as jt
from .mha import _canonical_mask,_none_or_dtype,baddbmm,scaled_dot_product_attention,_mha_shape_check,_in_projection_packed,_in_projection,multi_head_attention_forward,MultiheadAttention
...
class MyModule(Module):
    ...
    def register_parameter(self, name: str, param: jt.Var) -> None:
        if not isinstance(name, str):
            raise TypeError("Parameter name should be a string. Got {}".format(type(name)))
        if '.' in name:
            raise KeyError("Parameter name can't contain '.'")
        if name == '':
            raise KeyError("Parameter name can't be an empty string")
        if hasattr(self, name) and name not in self._parameters:
            raise KeyError("Attribute '{}' already exists".format(name))
        if param is not None and not isinstance(param, jt.Var):
            raise TypeError("Cannot assign '{}' object to parameter '{}' (jt.Var required)".format(type(param), name))
...
class LoRALayer(MyModule):
    ...
    def register_lora_param(self):
        for param_name, lora_name in self.params_with_lora.items():
            ...
            lora_A = jt.zeros((self.r, param.shape[1]), dtype=param.dtype)
            lora_B = jt.zeros((param.shape[0], self.r), dtype=param.dtype)

            self.register_parameter(f'{lora_name}_lora_A', lora_A)
            self.register_parameter(f'{lora_name}_lora_B', lora_B)
    ...
    def init_lora_param(self):
        for param_name, lora_name in self.params_with_lora.items():
            if hasattr(self, f'{lora_name}_lora_A'):
                jt.nn.init.kaiming_uniform_(getattr(self, f'{lora_name}_lora_A'), a=math.sqrt(5))
                jt.zeros_like(getattr(self, f'{lora_name}_lora_B'))
    ...
    def add_lora_data(self):
        for param_name, lora_name in self.params_with_lora.items():
            param = getattr(self, param_name)
            param.update(param + self.merge_BA(param_name) * self.scaling)
...
class Embedding(nn.Embedding, LoRALayer):
    ...
    def execute(self, x: jittor.Var, **kwargs):

        if self.r > 0 and not self.merged:
            self.merge_lora_param()
            result = nn.Embedding.execute(self, x, **kwargs)
            self.sub_lora_data()
            return result
        else:
            return nn.Embedding.execute(self, x, **kwargs)
...
class LinearLoRA(nn.Linear, LoRALayer):
    ...
    def execute(self, x: jittor.Var, **kwargs):
        ...
        if self.dropout is None: # do as before
            if self.r > 0 and not self.merged:
                self.merge_lora_param()
                result = nn.Linear.execute(self, x, **kwargs)
                self.sub_lora_data()
                return result
            else:
                return nn.Linear.execute(self, x, **kwargs)
            
        # Compute the original linear transformation
        original_output = nn.Linear.execute(self, x)

        if self.training and self.dropout.p > 0:
            x = self.dropout(x)
        
        if self.r > 0 and not self.merged:
            lora_adjustment = jt.matmul(x,self.merge_BA('weight').transpose(0, 1)) * self.scaling 
...
class Conv1d(nn.Conv1d, LoRALayer):
    ...
    def execute(self, x: jittor.Var, **kwargs):
        ...
        if self.r > 0 and not self.merged:
            self.merge_lora_param()
            result = nn.Conv1d.execute(self, x, **kwargs)
            self.sub_lora_data()
            return result
        else:
            return nn.Conv1d.execute(self, x, **kwargs)
...
class Conv2d(nn.Conv2d, LoRALayer):
    ...
    def execute(self, x: jittor.Var, **kwargs):
        ...
        if self.r > 0 and not self.merged:
            self.merge_lora_param()
            result = nn.Conv2d.execute(self, x, **kwargs)
            self.sub_lora_data()
            return result
        else:
            return nn.Conv2d.execute(self, x, **kwargs)
...
class Conv3d(nn.Conv3d, LoRALayer):
    ...
    def execute(self, x: jittor.Var, **kwargs):
        ...
        if self.r > 0 and not self.merged:
            self.merge_lora_param()
            result = nn.Conv3d.execute(self, x, **kwargs)
            self.sub_lora_data()
            return result
        else:
            return nn.Conv3d.execute(self, x, **kwargs)
...
class PlainMultiheadAttentionLoRA(nn.Module):
    def __init__(
        ...
        with jt.no_grad():
            ...
            existing_weight_tensor = jt.array(existing_weight)
            existing_bias_tensor = jt.array(existing_bias) if existing_bias is not None else None
            
            # Initialize q_proj
            self.q_proj.weight.data = existing_weight_tensor[:self.embed_dim, :]
            if existing_bias_tensor is not None:
                self.q_proj.bias.data = existing_bias_tensor[:self.embed_dim]
            # Initialize k_proj
            # Initialize k_proj
            self.k_proj.weight.data = existing_weight_tensor[self.embed_dim:2*self.embed_dim, :]
            if existing_bias_tensor is not None:
                self.k_proj.bias.data = existing_bias_tensor[self.embed_dim:2*self.embed_dim]
            
            # Initialize v_proj
            self.v_proj.weight.data = existing_weight_tensor[2*self.embed_dim:, :]
            if existing_bias_tensor is not None:
                self.v_proj.bias.data = existing_bias_tensor[2*self.embed_dim:]
    ...
    def execute(self,
        ...
        return self.execute_module(query, key, value, **kwargs) 
