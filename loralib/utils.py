...
def lora_state_dict(model: Module, bias: str = 'none') -> Dict[str, jittor.Var]:
...
def save_lora(args, list_lora_layers):
    ...
    save_path = f'{save_dir}/{args.filename}.pkl'
    jittor.save(save_data, save_path)
...
def load_lora(args, list_lora_layers):
    # to manage names like ViT-B/16
    backbone = args.backbone.replace('/', '').replace('-', '').lower()
    load_path = f'{args.save_path}/{backbone}/{args.dataset}/{args.shots}shots/seed{args.seed}/{args.filename}.pkl'

    if not os.path.exists(load_path):
        raise FileNotFoundError(f'File {load_path} does not exist.')

    loaded_data = jittor.load(load_path)
